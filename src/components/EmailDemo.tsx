...
import { Mail, RefreshCw, Send, Eye, Sparkles, Palette } from "lucide-react";
import { useEvents } from "@/contexts/EventsContext";
import { EmailTemplateModern } from "@/components/EmailTemplateModern";
...
export const EmailDemo = () => {
  ...
  const getTemplateComponent = () => {
    ...
    switch (selectedTemplate) {
      case "modern":
        return <EmailTemplateModern event={selectedEvent} />;
      case "incident":
  ...
  const getTemplateDescription = (template: string) => {
    switch (template) {
      case "modern":
        return "Modern design with gradients and cards - suitable for all event types";
      case "incident":
        ...
        <CardContent>
                ...
                <SelectContent>
                  <SelectItem value="modern">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <div className="font-medium">Modern Universal</div>
                        <div className="text-xs text-gray-500">Clean design for all events</div>
                      </div>
                    </div>
                  </SelectItem>
          ...
          {eventsWithEmail.length === 0 && (
            <div className="text-center text-gray-500 mt-4 p-8 border-2 border-dashed border-gray-200 rounded-lg bg-white">
              ...
              <p>Create an event with email notifications enabled to preview the new templates.</p>
            </div>
          )}
        </CardContent>
      </Card>
